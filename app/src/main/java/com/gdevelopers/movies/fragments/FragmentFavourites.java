...
public class FragmentFavourites extends KFragment implements UserMoviesAdapter.OnLoadMoreListener, ChangeMovieState.OnCallBackListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        favouritesRv.setLayoutManager(new GridLayoutManager(context, 3));

        TextView addWatchlistTv = rootView.findViewById(R.id.add_watchlist);
        TextView browseTv = rootView.findViewById(R.id.browse_text);
        browseTv.setText(R.string.browsing_lists_favourites);
        String src = getString(R.string.tap_favourites);
        SpannableString str = new SpannableString(src);
        int index = src.indexOf('@');
        Drawable d = ContextCompat.getDrawable(context, R.drawable.ic_action_favourite);
        assert d != null;
    ...
    public void serviceResponse(int responseID, List<KObject> objects) {
        ...
        if (responseID == Constants.GET_FAVOURITE_MOVIES && objects != null && !objects.isEmpty()) {
            if (!loadMore)
    ...
    public void onResume() {
        ...
        if (activity.getSupportActionBar() != null)
            activity.getSupportActionBar().setTitle(R.string.favourites);
        activity.getNavigationView().setCheckedItem(R.id.nav_favourites);
